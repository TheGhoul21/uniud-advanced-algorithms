...
(Figure~\ref{fig:substring-prefix-suffix}).
...
(Figure~\ref{fig:compaction-example}).
...
\aside{%
  ...
  see Figure~\ref{fig:stree-cacao} for the result on \texttt{cacao}.
}
...
$\mathrm{STrie}(T^{i-1})$ (Figure~\ref{fig:boundary-path}).
...
\caption{%
  ...
  character $t_i$.
  \textbf{Zone~1} (red): states $s_1, \ldots, s_{j-1}$ are leaves
  whose edges extend passively via open transitions --- no work needed.
  \textbf{Zone~2} (orange): states $s_j, \ldots, s_{j'-1}$ are
  non-leaf states that lack a $t_i$-transition --- the algorithm
  creates a new branch from each.
  \textbf{Zone~3} (green): from the endpoint $s_{j'}$ onwards, every
  state already has a $t_i$-transition, so the traversal stops.
  Only Zone~2 requires explicit work; the other two are either free or
  skipped entirely.
}

...
\node[font=\footnotesize\itshape, text=gray!60, anchor=west] at ({8*\cw}, -1.2) {mismatch at end of pattern};
\node[font=\footnotesize\itshape, text=gray!60, anchor=west] at ({8*\cw}, -2.6) {\texttt{E} $\notin P$: skip the entire block};
...
\begin{tikzpicture}[
    ...
    \node[below] at (9, -0.3) {\small Suffix $S$};
    ...
    \node[below] at (7.6, -0.3) {\scalebox{0.8}{Original}};

    \node[mismatch] at (2.6, 0) {$x$};
    \node[below] at (2.6, -0.3) {\scalebox{0.8}{Different!}};
...
To compute $L'(i)$ efficiently we exploit its connection with the
Z-algorithm applied to the reversed pattern $P^r$.
...
\begin{tikzpicture}[
    ...
    \draw[suffix] (2, 0) rectangle (3.5, 0.6) node[midway, font=\tiny] {$Z_k(P^r)$};
    \node[below] at (2, 0) {$k$};

    \draw[pbar] (0, -1.2) rectangle (6, -0.6);
    \node[left] at (0, -0.9) {$P$};

    \draw[suffix] (2.5, -1.2) rectangle (4, -0.6) node[midway, font=\tiny] {$N_j(P)$};
    \node[below] at (4, -1.2) {$j$};

    \draw[<->, thick, Stealth-Stealth, blue!60!black]
      (3, -0.1) -- (3, -0.5)
      node[midway, right, font=\tiny\itshape] {reversal correspondence};
\end{tikzpicture}
\end{center}
...
The classical worst case is $T = a^n$, $P = ba^{m-1}$: every alignment
compares the entire pattern right-to-left, reaches the first character, and
shifts by only one position.

In contrast, for a random text over a large alphabet, the algorithm
frequently triggers the BCR skip, leading to an average-case complexity of
$\Oh{n/m}$.

\subsubsection{Apostolico--Giancarlo Optimization}

The Boyer-Moore algorithm can perform $\Oh{nm}$ comparisons in the worst case because it "forgets" matches made in previous alignments. The \textbf{Apostolico--Giancarlo} algorithm improves this to $\Oh{n}$ by maintaining an array $M$ where $M[k]$ is the length of the longest suffix of $P$ that matched $T$ ending at position $k$ in some previous alignment.
...
single pattern.
The solution is to equip $K(\mathcal{P})$ with \textbf{failure links},

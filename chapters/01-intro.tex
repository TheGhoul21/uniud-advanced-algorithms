...
\begin{example}[BCR skip]
  ...
  $P[4] = \texttt{O}$ vs $T[4] = \texttt{E}$ è un mismatch.
  Siccome \texttt{E} non compare in $P$, la BCR sposta l'intero pattern oltre il carattere "cattivo".
...
% Griglia verticale di riferimento
\foreach \x in {0,...,7} {
...
\node[font=\footnotesize\itshape, text=gray!60, anchor=west] at ({8*\cw}, -1.2) {mismatch alla fine del pattern};
\node[font=\footnotesize\itshape, text=gray!60, anchor=west] at ({8*\cw}, -2.6) {\texttt{E} $\notin P$: saltiamo l'intero blocco};

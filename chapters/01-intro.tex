...
\begin{example}[GSR intuition]
  Let $P = \texttt{CABAB}$ and suppose scanning right-to-left we match
  $S = P[4\ldots5] = \texttt{AB}$ against $T$, then mismatch at $P[3]=\texttt{B}$.
  The substring $\texttt{AB}$ also appears in $P$ at positions $2$--$3$, preceded
  by $\texttt{C} \neq \texttt{B}$ (so the same mismatch cannot immediately
  recur). We shift $P$ right by $2$ to realign that copy of $\texttt{AB}$
  with the matched region of $T$.
\end{example}
...
\begin{tikzpicture}[
    ...
    \node[below] at (9, -0.3) {\small Suffix $S = P[i\ldots m]$};

    \draw[suffix] (3, -0.3) rectangle (5, 0.3);
    \node[above] at (5, 0.3) {$L'(i)$};
    \node[below] at (4, -0.3) {\small Copy of $S$};

    \node[mismatch] at (7.6, 0) {$y$};
    \node[below] at (7.6, -0.3) {\scalebox{0.8}{mismatch char}};

    \node[mismatch] at (2.6, 0) {$x$};
    \node[below] at (2.6, -0.3) {\scalebox{0.8}{$x \neq y$: safe!}};
...
\subsubsection{Apostolico--Giancarlo Optimization}
...
\subsection{Generalization of the Problem: Multiple Patterns}
...
% ══════════════════════════════════════════════════════════════════
\subsubsection{Using $K(\mathcal{P})$ on $T$}
% ══════════════════════════════════════════════════════════════════

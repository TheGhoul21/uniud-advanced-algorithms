...
\begin{example}[Building a keyword tree]
  Let $\mathcal{P} = \{\texttt{potato},\texttt{tatoo},\texttt{theater},\texttt{other}\}$.
...
\begin{tikzpicture}[
  ...
  %% ── SHARED-PREFIX BRACE ──────────────────────────────────────
  % Span across nTA and nTH (sibling depth-2 nodes under ndT)
  % mirror → brace opens downward; label sits below the two subtrees
  \draw[blue!35, thick, decorate,
        decoration={brace, mirror, raise=5pt, amplitude=5pt}]
    (nTA.west) -- (nTH.east)
    node[midway, below=13pt, font=\tiny\sffamily, text=blue!55!black]
      {share prefix \texttt{t} $\to$ tracked by one node};
...
The tree is built once in $\Oh{m}$ time.
We then make a \emph{single} left-to-right pass over $T$: at each starting
position $s$ we descend $K(\mathcal{P})$ from the root following edge-labels
$T[s], T[s+1], \ldots$; reaching a leaf at depth $m_i$ reports an occurrence
of $P_i$ ending at $T[s + m_i - 1]$.

\aside{The key gain: all patterns sharing a common prefix are tracked
\emph{in parallel} by a single internal node.
As long as the text characters keep matching a shared prefix, no extra
work is done for any of them --- the tree compresses the joint
prefix-matching state for the whole set $\mathcal{P}$.}
...
Consider searching for $\mathcal{P} = \{\texttt{he},\texttt{she},\texttt{his},\texttt{hers}\}$
in $T = \texttt{ushers}$.
After reading \texttt{u} from the root we fail immediately (no \texttt{u}-edge),
restart from the root, try \texttt{s} → \texttt{h} → \texttt{e} → leaf
(\texttt{she} found). Now the \emph{naive} tree traversal restarts from the
root, but the \texttt{e} we just read is itself the start of \texttt{he}!
We missed a match because we threw away the suffix information.

This is exactly the problem KMP solved for a single pattern with the $sp'$
table: instead of restarting from scratch, jump to the longest proper border
of the current prefix and continue from there.
For multiple patterns the same idea generalises: at a mismatch (or after a
full match) we follow a \textbf{failure link} from the current node to the
node representing the longest proper suffix of the current prefix that is
also a prefix of some pattern in $\mathcal{P}$.
These failure links turn $K(\mathcal{P})$ into a finite automaton that
processes $T$ in a single left-to-right pass, never re-reading any character.

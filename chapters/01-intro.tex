...
\begin{example}[BCR skip]
  ...
  Since \texttt{E} does not appear anywhere in $P$, BCR shifts the entire
  pattern past the mismatch character.
...
\begin{tikzpicture}[
  ...
  T/.style={cell, draw=gray!30, fill=gray!5},
  ...
  Pnew/.style={cell, draw=green!50!black, fill=green!10},
]
\def\cw{0.85}
...
\node[T]         at ({4*\cw}, 0) {R};
\node[T]         at ({5*\cw}, 0) {O};
\node[T, draw=none, fill=none] at ({6*\cw}, 0) {\dots};

\node[font=\small\sffamily, text=gray!60, anchor=east] at ({-0.5*\cw}, -1) {$P$};
\node[P]         at ({0*\cw}, -1) {A};
\node[P]         at ({1*\cw}, -1) {L};
\node[P]         at ({2*\cw}, -1) {G};
\node[MismatchP] at ({3*\cw}, -1) {O};

\node[font=\tiny\sffamily\bfseries, text=red!70!black] at ({3*\cw}, -0.5) {$\times$};

\draw[->, thick, gray!80, rounded corners=4pt]
  ({1.5*\cw}, -1.5) -- ++(0, -0.4) -| ({5.5*\cw}, -2.6);

\node[font=\footnotesize\sffamily, text=gray!70, text width=5.5cm, align=center]
  at ({3.5*\cw}, -2.1)
  {\texttt{E} does not appear in $P$: skip the entire pattern past it.};

\node[font=\small\sffamily, text=gray!60, anchor=east] at ({3.5*\cw}, -3.1) {$P'$};
\node[Pnew] at ({4*\cw}, -3.1) {A};
\node[Pnew] at ({5*\cw}, -3.1) {L};
\node[Pnew] at ({6*\cw}, -3.1) {G};
\node[Pnew] at ({7*\cw}, -3.1) {O};
...
Two standard approaches exist for building the BCR table:
\begin{enumerate}[label=\alph*)]
  \item \textbf{Full 2-D table} ($|\Sigma| \times m$): $\Oh{1}$ query time,
        but $\Oh{|\Sigma| m}$ space.
  \item \textbf{Position lists}: for each character store the sorted list of
        its occurrences in $P$ and answer queries with binary search in
        $\Oh{\log m}$.
\end{enumerate}

\begin{remark}
  Although binary search costs $\Oh{\log m}$ per query, the large shifts
  that BCR produces amortise this overhead over the full run of the
  algorithm, making the two implementations equivalent in practice.
\end{remark}
...
\begin{remark}
  The \textbf{Apostolico--Giancarlo} extension records which portions of
  the text have already been matched, avoiding redundant comparisons and
  bringing the worst-case complexity down to $\Oh{n+m}$.
  On the other hand, in favourable inputs (e.g.\ $T = c^n$,
  $P = a^{m-1}c$) BCR allows Boyer-Moore to scan the text in as few as
  $\lceil n/m \rceil$ character comparisons---sublinear in $n$.
\end{remark}

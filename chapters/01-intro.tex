...
\draw[->, >=Stealth, thick, gray!50] ({3.4*\cw}, -0.85) -- ({-0.2*\cw}, -0.85)
  node[midway, above, font=\scriptsize\sffamily, text=gray!60] {scan direction};
...
\draw[->, >=Stealth, thick, gray!50]
  ({4.4*\cw}, -0.6) -- ({-0.2*\cw}, -0.6)
  node[midway, above, font=\scriptsize\sffamily, text=gray!60]
    {scan direction};
...
\begin{tikzpicture}[
    scale=0.9,
    pbar/.style={draw, fill=blue!5, thick, rounded corners=2pt},
    suffix/.style={fill=orange!20, draw=orange!70, thick},
]
    \draw[pbar] (0, 0) rectangle (6, 0.6);
    \node[left] at (0, 0.3) {$P^r$};

    \draw[suffix] (2, 0) rectangle (3.5, 0.6) node[midway, font=\tiny] {$Z_k(P^r) = \ell$};
    \node[below] at (2, 0) {$k$};

    \draw[pbar] (0, -1.2) rectangle (6, -0.6);
    \node[left] at (0, -0.9) {$P$};

    \draw[suffix] (2.5, -1.2) rectangle (4, -0.6) node[midway, font=\tiny] {$N_j(P) = \ell$};
    \node[below] at (4, -1.2) {$j$};

    \draw[<->, thick, Stealth-Stealth, blue!60!black]
      (3, -0.1) -- (3, -0.5)
      node[midway, right, font=\tiny\itshape] {$k = m{-}j{+}1$};
\end{tikzpicture}
\caption{Reversal correspondence: $N_j(P) = Z_{m-j+1}(P^r)$.
  A Z-match of length $\ell$ at position $k$ in the reversed pattern
  corresponds to a suffix-match of length $\ell$ ending at position $j = m-k+1$
...
\begin{tikzpicture}[
  ...
  \node[anchor=north west, font=\footnotesize\sffamily] at (7.5, -1.0) {
    \begin{tabular}{@{}l@{}}
      ...
      \textcolor{red!50!black}{$\dashrightarrow$\;failure link}
    \end{tabular}
  };

...
\begin{note}[{A fallback that succeeds}]
  ...
  The longest border of $\texttt{abab}$ is $\texttt{ab}$ (length~$2$), so $sp[4]=2$.
...
\begin{formaldetails}[{Why no candidate between $sp[k]$ and $k$ exists}]
  Suppose for contradiction that $sp[k] < k' < k$ and that $k'$ satisfies
  ...
  $P[1\ldots k']$ a border of $P[1\ldots k]$ of length $k' > sp[k]$,
  contradicting $sp[k]$ being the \emph{longest} proper border of
  $P[1\ldots k]$. \hfill$\square$
\end{formaldetails}

\begin{formaldetails}[Correctness of the $sp$ preprocessing algorithm]
  We prove that at the end of iteration~$i$, $sp[i]$ equals the length of the
  longest proper border of $P[1\ldots i]$.

  \begin{loopproof}
    \item[Precondition]  $P[1\ldots m]$ is given; $sp[1]=0$ and $k=0$ are set before the loop.
    ...
    \item[Invariant]     At the \emph{start} of iteration~$i$:
                         ...
                         $P[1\ldots k] = P[i{-}k\ldots i{-}1]$).
    \item[Postcondition] $sp[i]$ is correctly set for all $1 \le i \le m$.
  \end{loopproof}

  \lprulesep

  \begin{loopproof}
    \item[Base case]     Before the first iteration ($i=2$), $k=0=sp[1]$
    ...
    \item[Preservation]  Assume the invariant holds at the start of iteration~$i$,
                         ...
                         $k \to sp[k] \to sp[sp[k]] \to \cdots$, stopping as soon
                         as $P[k{+}1]=P[i]$ or $k=0$.
                         ...
                         leaves $k=0$.
                         In both cases $sp[i] \gets k$ is correct, and $k=sp[i]$
                         is the invariant for iteration $i{+}1$. \checkmark
    \item[Postcondition] When the loop exits ($i=m+1$), all $sp[i]$ have been
...
\node[SPhi, minimum height=5mm] at ({3*\cw}, 0.95) {2};   % sp[4]=2, key value
\node[SP,   minimum height=5mm] at ({4*\cw}, 0.95) {0};
\node[font=\tiny\sffamily, text=orange!60!black, anchor=west]
  at ({5.0*\cw}, 0.95) {$sp[4]{=}2$: longest border of \texttt{abab} is \texttt{ab}
...
% ── Phase 1: offset=0, 4 new matches, fail at T[5] ────────
\node[font=\scriptsize\sffamily, text=gray!55, anchor=east]
  at ({-0.5*\cw},{-1.35}){Ph.\,1};
...
\node[font=\tiny\sffamily, text=orange!62!black, anchor=west]
  at ({4.2*\cw},{-1.85}){$j\gets sp[4]{=}2$};

% ── Phase 2: offset=2, 2 carry + 2 new, fail at T[7] ──────
\node[font=\scriptsize\sffamily, text=gray!55, anchor=east]
  at ({-0.5*\cw},{-2.40}){Ph.\,2};
...
\draw[teal!50!black, decorate,
  ...
  node[midway, below=8pt, font=\tiny\sffamily, text=teal!50!black]
    {free via $sp$};
\node[font=\scriptsize\sffamily, text=gray!55, anchor=west]
  at ({9.2*\cw},{-2.40}){$T[7]{=}\texttt{a}\neq P[5]{=}\texttt{c}$};

% fallback arrow Phase 2 → Phase 3
\draw[->, orange!65!black, shorten >=2pt, shorten <=2pt]
  ({6*\cw},{-2.67}) -- ({4*\cw},{-3.13});
\node[font=\tiny\sffamily, text=orange!62!black, anchor=west]
  at ({6.2*\cw},{-2.90}){$j\gets sp[4]{=}2$};
...
\caption{%
  ...
  phase via $j\gets sp[j]$.
  \textcolor{red!55!black}{Red}: mismatch.
  After each fallback, the algorithm reuses the last $sp[j]$ matched characters
...
\foreach \j in {1,...,12}
  \node[font=\tiny\sffamily, text=gray!40] at ({(\j-1)*\cw}, 0.62) {\j};
...
% labels above
\node[font=\tiny\sffamily, text=teal!60!black]    at ({6*\cw},  0.43){$l$};
\node[font=\tiny\sffamily, text=orange!65!black]  at ({2*\cw},  0.43){$k$};
\node[font=\tiny\sffamily, text=orange!65!black]  at ({8*\cw},  0.43){$i$};
\node[font=\tiny\sffamily, text=teal!60!black]    at ({10*\cw}, 0.43){$r$};
\node[font=\tiny\sffamily, text=gray!50]          at ({11*\cw}, 0.43){$r{+}1$};

% mirror arrow k ↔ i
\draw[orange!58!black, <->, shorten >=3pt, shorten <=3pt]
  ({2*\cw}, 0.43) to[bend left=38]
  node[above=1pt, font=\tiny\sffamily, text=orange!58!black]{mirror: $k=i{-}l{+}1$}
  ({8*\cw}, 0.43);
...
\node[font=\tiny\sffamily, text=orange!60!black] at ({2*\cw},{-2.10+0.43}){$k$};
\node[font=\tiny\sffamily, text=orange!60!black] at ({8*\cw},{-2.10+0.43}){$i$};
\node[font=\normalsize, text=red!55!black] at ({4*\cw},{-2.10+0.42}){$\times$};
\node[font=\normalsize, text=red!55!black] at ({10*\cw},{-2.10+0.42}){$\times$};

\draw[green!50!black, decorate, decoration={brace, mirror, amplitude=3pt, raise=2pt}]
  ({2*\cw-0.37},{-2.10-0.33}) -- ({3*\cw+0.37},{-2.10-0.33})
  node[midway, below=8pt, font=\tiny\sffamily, text=green!50!black]{$Z[k]$};
\draw[green!50!black, decorate, decoration={brace, mirror, amplitude=3pt, raise=2pt}]
  ({8*\cw-0.37},{-2.10-0.33}) -- ({9*\cw+0.37},{-2.10-0.33})
  node[midway, below=8pt, font=\tiny\sffamily, text=green!50!black]{$Z[k]$};

\node[font=\footnotesize\sffamily, text=gray!60, anchor=west, text width=3.8cm, align=left]
  at ({12.2*\cw},{-2.65}){$Z[k]<r{-}i{+}1 \;\Rightarrow\; Z[i]=Z[k]$, no comparison};

% separator
\draw[gray!18] ({-0.5*\cw},{-3.85}) -- ({11.5*\cw},{-3.85});

% ── CASE (b): Z[k]=3 ≥ r-i+1=3 ──────────────────────────
\node[font=\scriptsize\sffamily\bfseries, text=gray!55, anchor=east]
  at ({-0.5*\cw},{-4.55}){(b)};
...
\node[font=\tiny\sffamily, text=orange!60!black] at ({2*\cw},{-3.65+0.43}){$k$};
\node[font=\tiny\sffamily, text=orange!60!black] at ({8*\cw},{-3.65+0.43}){$i$};
\node[font=\tiny\sffamily, text=teal!55!black]   at ({10*\cw},{-3.65+0.43}){$r$};
\node[font=\tiny\sffamily, text=gray!50]         at ({11*\cw},{-3.65+0.43}){$r{+}1$};

\draw[->, gray!55, thick, shorten <=3pt]
  ({11*\cw},{-4.55}) -- ({12.5*\cw},{-4.55})
  node[right, font=\tiny\sffamily, text=gray!55]{extend};

\draw[green!50!black, decorate, decoration={brace, mirror, amplitude=3pt, raise=2pt}]
  ({2*\cw-0.37},{-3.65-0.33}) -- ({4*\cw+0.37},{-3.65-0.33})
  node[midway, below=8pt, font=\tiny\sffamily, text=green!50!black]{$Z[k]\geq r{-}i{+}1$};
\draw[green!50!black, decorate, decoration={brace, mirror, amplitude=3pt, raise=2pt}]
  ({8*\cw-0.37},{-3.65-0.33}) -- ({10*\cw+0.37},{-3.65-0.33})
  node[midway, below=8pt, font=\tiny\sffamily, text=green!50!black]{$r{-}i{+}1$ (free)};

\node[font=\footnotesize\sffamily, text=gray!60, anchor=west, text width=3.8cm, align=left]
  at ({12.2*\cw},{-4.55}){$Z[k]\geq r{-}i{+}1 \;\Rightarrow\; Z[i]\geq r{-}i{+}1$, compare from $r{+}1$};
...
\caption{%
  ...
  \textcolor{green!55!black}{Green}: the $Z[k]$ characters that match at both $k$ and $i$
  (free, by the mirror property).
  \textcolor{red!60!black}{Red ×}: the mismatch that terminated $Z[k]$, reflected at~$i$.
  Case~(a): the mismatch lies inside $[l,r]$, so $Z[i]=Z[k]$ at no cost.
...
\end{figure}
The following two cases (Figure~\ref{fig:z-box} and Figure~\ref{fig:z-trace}) distinguish how we
compute $Z_i$:
...
Figure~\ref{fig:z-trace} traces the full execution on $A = \texttt{aabaaab}$,
showing how $[l,r]$ evolves across all three cases.

% ── Figure: Z-box trace (full step-by-step) ──────────────
\begin{figure}[H]
\centering
\begin{tikzpicture}[
  cell/.style={
...
% --- column index header ---
\foreach \j in {1,...,7}
  \node[font=\tiny\sffamily, text=gray!45] at ({(\j-1)*\cw}, 0.65) {\j};

% ── i=2 : case 1, no box yet (l=r=0) ──────────────────────
\node[font=\small\sffamily, text=gray!65, anchor=east] at ({-0.5*\cw},0){$i{=}2$};
\node[NC]  at ({0*\cw},0){a}; \node[Cur] at ({1*\cw},0){a};
\node[NC]  at ({2*\cw},0){b}; \node[NC]  at ({3*\cw},0){a};
\node[NC]  at ({4*\cw},0){a}; \node[NC]  at ({5*\cw},0){a};
\node[NC]  at ({6*\cw},0){b};
\node[font=\tiny\sffamily, text=orange!70!black] at ({1*\cw}, 0.47) {$i$};
\node[font=\footnotesize\sffamily, text=gray!60, anchor=west]
  at ({7.2*\cw},0){case~1\;$\to$\;$Z[2]{=}1$,\quad $[l,r]\gets[2,2]$};

% ── i=4 : case 1, [l,r]=[2,2] entering ───────────────────
\node[font=\small\sffamily, text=gray!65, anchor=east] at ({-0.5*\cw},{-\rs}){$i{=}4$};
\node[NC]  at ({0*\cw},{-\rs}){a}; \node[Box] at ({1*\cw},{-\rs}){a};
\node[NC]  at ({2*\cw},{-\rs}){b}; \node[Cur] at ({3*\cw},{-\rs}){a};
\node[NC]  at ({4*\cw},{-\rs}){a}; \node[NC]  at ({5*\cw},{-\rs}){a};
\node[NC]  at ({6*\cw},{-\rs}){b};
\node[font=\tiny\sffamily, text=orange!70!black] at ({3*\cw},{-\rs+0.47}){$i$};
\node[font=\footnotesize\sffamily, text=gray!60, anchor=west]
  at ({7.2*\cw},{-\rs}){case~1\;$\to$\;$Z[4]{=}2$,\quad $[l,r]\gets[4,5]$};

% ── i=5 : case 2b, [l,r]=[4,5], k=2 ─────────────────────
\node[font=\small\sffamily, text=gray!65, anchor=east] at ({-0.5*\cw},{-2*\rs}){$i{=}5$};
\node[NC]   at ({0*\cw},{-2*\rs}){a}; \node[Mir]  at ({1*\cw},{-2*\rs}){a};
\node[NC]   at ({2*\cw},{-2*\rs}){b}; \node[Box]  at ({3*\cw},{-2*\rs}){a};
\node[CurB] at ({4*\cw},{-2*\rs}){a}; \node[NC]   at ({5*\cw},{-2*\rs}){a};
\node[NC]   at ({6*\cw},{-2*\rs}){b};
\node[font=\tiny\sffamily, text=violet!60!black] at ({1*\cw},{-2*\rs+0.47}){$k$};
\node[font=\tiny\sffamily, text=orange!70!black] at ({4*\cw},{-2*\rs+0.47}){$i$};
\draw[violet!50!black, <->, shorten >=3pt, shorten <=3pt]
  ({1*\cw},{-2*\rs+0.35}) to[bend left=40]
  node[midway, below=0.5pt, font=\tiny\sffamily, text=violet!55!black]{mirror}
  ({4*\cw},{-2*\rs+0.35});
\node[font=\footnotesize\sffamily, text=gray!60, anchor=west]
  at ({7.2*\cw},{-2*\rs}){case~2b\;$\to$\;$Z[5]{=}3$,\quad $[l,r]\gets[5,7]$};

% ── i=6 : case 2a, [l,r]=[5,7], k=2 ─────────────────────
\node[font=\small\sffamily, text=gray!65, anchor=east] at ({-0.5*\cw},{-3*\rs}){$i{=}6$};
\node[NC]   at ({0*\cw},{-3*\rs}){a}; \node[Mir]  at ({1*\cw},{-3*\rs}){a};
\node[NC]   at ({2*\cw},{-3*\rs}){b}; \node[NC]   at ({3*\cw},{-3*\rs}){a};
\node[Box]  at ({4*\cw},{-3*\rs}){a}; \node[CurB] at ({5*\cw},{-3*\rs}){a};
\node[Box]  at ({6*\cw},{-3*\rs}){b};
\node[font=\tiny\sffamily, text=violet!60!black] at ({1*\cw},{-3*\rs+0.47}){$k$};
\node[font=\tiny\sffamily, text=orange!70!black] at ({5*\cw},{-3*\rs+0.47}){$i$};
\draw[violet!50!black, <->, shorten >=3pt, shorten <=3pt]
  ({1*\cw},{-3*\rs+0.35}) to[bend left=35]
  node[midway, below=0.5pt, font=\tiny\sffamily, text=violet!55!black]{mirror}
  ({5*\cw},{-3*\rs+0.35});
\node[font=\footnotesize\sffamily, text=gray!60, anchor=west]
  at ({7.2*\cw},{-3*\rs}){case~2a\;$\to$\;$Z[6]{=}Z[k]{=}1$,\quad $[l,r]$ unchanged};

% ── separator ─────────────────────────────────────────────
\draw[gray!20] ({-0.6*\cw},{-3.75*\rs}) -- ({6.6*\cw},{-3.75*\rs});

% ── final Z values ────────────────────────────────────────
\node[font=\small\sffamily, text=gray!60, anchor=east] at ({-0.5*\cw},{-4.25*\rs}){$Z$};
\node[Zv] at ({0*\cw},{-4.25*\rs}){---};
\node[Zh] at ({1*\cw},{-4.25*\rs}){1};
\node[Zv] at ({2*\cw},{-4.25*\rs}){0};
\node[Zh] at ({3*\cw},{-4.25*\rs}){2};
\node[Zh] at ({4*\cw},{-4.25*\rs}){3};
\node[Zh] at ({5*\cw},{-4.25*\rs}){1};
\node[Zv] at ({6*\cw},{-4.25*\rs}){0};
\end{tikzpicture}
\caption{%
  Z-box algorithm on $A=\texttt{aabaaab}$ (steps $i{=}3$ and $i{=}7$ omitted: $Z{=}0$, no update).
  \textcolor{teal!65!black}{Teal}: current Z-box $[l,r]$.
  \textcolor{orange!75!black}{Orange border}: current position $i$
  (filled teal when $i$ is inside the box).
  \textcolor{violet!60!black}{Violet}: mirror $k = i{-}l{+}1$, with arc showing
...
\begin{formaldetails}[Correctness of the Z-box algorithm]
  We prove that the algorithm sets $Z[i] = Z_i(A)$ for every $i \ge 2$.

  \begin{loopproof}
    \item[Precondition]  $A[1\ldots n]$ is given; $l = r = 0$ before the loop.
    \item[Loop guard]    $i$ ranges from $2$ to $n$.
    \item[Invariant]     At the \emph{start} of iteration~$i$:
                         (i)~$Z[j]$ is correctly set for all $2 \le j < i$, and
    ...
    \item[Postcondition] $Z[i] = Z_i(A)$ for all $i \in \{2, \ldots, n\}$.
  \end{loopproof}
  ...
  \begin{loopproof}
    ...
    \item[Preservation]  Assume the invariant holds at the start of iteration~$i$.
                         ...
                         mismatch, so $Z[i]$ is set to the correct value by
                         definition. If $Z[i] > 0$, the box is updated to
                         $[l, r] = [i,\, i{+}Z[i]{-}1]$, which has a larger $r$
                         than before. \checkmark

                         \textit{Case 2a: $i \le r$ and $Z[k] < r{-}i{+}1$
                         (mirror strictly inside the box).}
                         Let $k = i{-}l{+}1$.
                         From the invariant, $A[l\ldots r] = A[1\ldots r{-}l{+}1]$,
                         so $A[i{+}j] = A[k{+}j]$ for $j = 0, \ldots, r{-}i$.
                         In particular $A[i\ldots i{+}Z[k]{-}1] = A[k\ldots k{+}Z[k]{-}1] = A[1\ldots Z[k]]$.
                         Since $Z[k] < r{-}i{+}1$, position $k{+}Z[k]$ still lies
                         inside $[1, r{-}l{+}1]$, so
                         $A[i{+}Z[k]] = A[k{+}Z[k]] \ne A[1{+}Z[k]]$
                         ...
                         \textit{Case 2b: $i \le r$ and $Z[k] \ge r{-}i{+}1$
                         (mirror reaches or exceeds the box boundary).}
                         The same translation gives $A[i\ldots r] = A[1\ldots r{-}i{+}1]$,
                         so $Z[i] \ge r{-}i{+}1$ is guaranteed.
                         The algorithm initialises $Z[i] = r{-}i{+}1$ and extends
                         it by comparing $A[r{+}1], A[r{+}2], \ldots$ directly
                         until a mismatch, exactly as in Case~1.
                         The box is then updated to $[i,\, i{+}Z[i]{-}1]$. \checkmark

    \item[Postcondition] When the loop exits, all $Z[i]$ have been correctly
